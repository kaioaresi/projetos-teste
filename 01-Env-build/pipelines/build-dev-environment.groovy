            ...
            steps {     
              script {
                if (!params.SKIP_STEP_1){    
                    ...
                    sh """
                    sed 's/<PASSOWRD>/$params.MYSQL_PASSWORD/g' include/create_developer.template > include/create_developer.sql
                    """
        ...
        stage('Start new container using latest image and create user') {
            steps {     
              script {
                ...
                sh """
                docker container exec ${containerName} /bin/bash -c 'mysql --user="root" --password="$params.MYSQL_PASSWORD" < /scripts/create_developer.sql'
                """
